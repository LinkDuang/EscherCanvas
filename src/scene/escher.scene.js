...
class EscherScene {
  ...
  autoRegisterCanvas(select = 'canvas') {
    let canvasDoms = document.querySelectorAll(select)
    if (canvasDoms.length > 1) {
      throw Error(
        'Make sure there is only one canvas element in the UI, or pass in the exact selector to specify one of them. 请确保界面中只有一个 canvas 元素，或传入准确的选择器来指定其中一个。',
      )
    } else if (canvasDoms.length === 0) {
      throw Error(
        'The canvas element was not found, check the incoming selector. 未找到 canvas 元素，请检查传入的选择器。',
      )
    }
    let canvas = canvasDoms[0]
    this.canvas = canvas
